// Map of spelt out digits to their number-strings
const speltToNumberString = new Map([
    ['zero', '0'],
    ['one', '1'],
    ['two', '2'],
    ['three', '3'],
...
const MAX_DIGIT_LENGTH = 5;

// O(N*M), N is number of lines, M is length of longest string.
// Since we only check 5 characters max for each index, we only multiply a constant to this time.
function calibrationValuesPart2(lines: string[]): number {
    function findSpeltDigitFromIndex(line: string, index: number): string | undefined {
        // Start from index, and step until we have spelt a number
        let digit = '';
        for (let i = index; i < index + MAX_DIGIT_LENGTH; i++) {
            digit += line.charAt(i);
            if (speltToNumberString.has(digit)) {
                return speltToNumberString.get(digit);
            }
        }
        return undefined;
    }

    // Get calibrarion values from each line
    function getValueFromString(line: string): number {
        let firstDigit = '0';
        let lastDigit = '0';

        // Start from the front, find a digit
        for (let i = 0; i < line.length; i++) {
            if (Number.isInteger(Number(line.charAt(i)))) {
                firstDigit = line.charAt(i);
                break;
            }
            const speltDigit = findSpeltDigitFromIndex(line, i);
            if (speltDigit) {
                firstDigit = speltDigit;
                break;
            }
        }

        // Start from the back, find a digit
...
console.log(calibrationValuesPart2(input));
