...
function sumOfGamePowers(gameString: string): number {
    ...
    return gameString
        ...
        .map((game: string): string[] => {
            const [_, gameContent] = game.split(':', 2);
            // 1.c. Separate each grab reveal
            const grabReveals =  trimStringArray(gameContent.split(';'));
            return grabReveals;
        })
        .map((grabReveals): number => {
            const maximumCubes: {[key: string]: number} = 
                grabReveals
                    ...
                    // Get maximum cubes for game
                    .reduce((maximumCubes: {[key: string]: number}, revealCubeCounts: {[key: string]: number}): {[key: string]: number} => {
                        // Count maximum of each color cube revealed
                        for (let color in revealCubeCounts) {
                            if (!maximumCubes[color]) {
                                maximumCubes[color] = revealCubeCounts[color];
                            } else if (maximumCubes[color] < revealCubeCounts[color]) {
                                maximumCubes[color] = revealCubeCounts[color];
                            }
                        }
                        return maximumCubes;
            ...
            // Get game's power
            let power = 1;
            for (let color in maximumCubes) {
                power *= maximumCubes[color];
            }
            return power;
        })
        // Sum up game powers
        .reduce((sum, power): number => sum + power, 0);
}
...
console.log(sumOfGamePowers(input));
