...
function trimStringArray(stringArray: string[]): string[] {
    return stringArray.map((singleString) => singleString.trim());
}

function sumOfPossibleGames(gameString: string, bagCubeColors: {[key: string]: number}): number {
    ...
    return gameString
        ...
        // 1.b. Parse game IDs
        .map((game: string): [number, string[]] => {
            const [gameIdPart, gameContent] = game.split(':', 2);
            const gameId = Number(gameIdPart.split(' ', 2)[1]);
            // 1.c. Separate each grab reveal
            const grabReveals =  trimStringArray(gameContent.split(';'));
            return [gameId, grabReveals];
        })
        .map(([gameId, grabReveals]): number => {
            const possible: boolean = 
                grabReveals
                    .map((grabReveal: string): {[key: string]: number} => {
                        // 1.d. Parse each reveal
                        const grabContents: string[] = trimStringArray(grabReveal.split(','));
                        // 1.e. Parse out each item in a hand
                        const cubeCounts: {[key: string]: number} = 
                            grabContents
                                .map((grabContent): {[key: string]: number} => {
                                    const [countString, color]: string[] = grabContent.split(' ', 2);
                                    const count = Number(countString);
                                    return {[color]: count};
                                })
                                // Accumulate into counts by color
                                .reduce((cubeCounts, singleColorCounts) => {
                                    return {
                                        ...cubeCounts,
                                        ...singleColorCounts,
                                    };
                    ...
                    // 1.f. Check if every reveal is possible
                    .every((revealCubeCounts: {[key: string]: number}): boolean => {
                        // Check each cube color revealed
                        for (let color in revealCubeCounts) {
                            if (!bagCubeColors[color]) {
                                return false;
                            } else if (bagCubeColors[color] < revealCubeCounts[color]) {
                                return false;
                            }
                        }
                        return true;
        ...
        .reduce((sum, gameId): number => sum + gameId, 0);
}

// Input provided by problem
const input: string = `
...
// Cube colors provided by Elf
const possibleBagCubeColors: {[key: string]: number} = {
    'red': 12,
    'green': 13,
    'blue': 14,
};

console.log(sumOfPossibleGames(input, possibleBagCubeColors));
