...
/** For a source number, figure out how to convert it to the destination number based on ranges */
function convertSourceToDestination(sourceNumber: number, mappings: MappingRange[]): number {
  // Check each range, see if sourceNumber is included in range
  // What if there's overlap in ranges? I assume there aren't any overlaps, since I don't know how to prioritize.
  for (let mapping of mappings) {
    const {destinationStart, sourceStart, rangeLength} = mapping;
    if (rangeLength === 0) {
      continue;
    }
    if (sourceStart <= sourceNumber && (sourceStart + rangeLength) > sourceNumber) {
      // sourceNumber is in this range
      const offset = sourceNumber - sourceStart;
      return destinationStart + offset;
    }
  }
  // Default to being the same value
  return sourceNumber;
}

/**
 * For an array of numbers of range start and range lengths,
 * return a list of ranges bundled together
 */
function getSeedRangesFromStartsAndLengths(startsAndLengths: number[]): NumberRange[] {
  const seedRanges: NumberRange[] = [];
  // Walk through array 2 indices at a time
  for (let rangeStartIndex = 0; (rangeStartIndex + 1) < startsAndLengths.length; rangeStartIndex += 2) {
    const start = startsAndLengths[rangeStartIndex];
    const length = startsAndLengths[rangeStartIndex + 1];
    seedRanges.push({
      start,
      length,
    });
...
function lowestLocationNumberForSeeds(almanac: string): number {
  const [seedsString, ...mapsStrings]: string[] = almanac.split('\n\n').filter((section) => section.length > 0);
  const seedRangesStartsAndLengths: number[] = seedsString
    .split(':')[1]
    .split(' ')
    .filter((strings) => strings.length > 0)
    .map((seedString) => Number(seedString));
  const seedRanges: NumberRange[] = getSeedRangesFromStartsAndLengths(seedRangesStartsAndLengths);
  
  const almanacMaps: AlmanacMapping[] = mapsStrings
    .map((mapString) => {
      const [mapDefinitionString, mappingRangeLines] = mapString
        .split(':');

      const [sourceName, , destinationName]: string[] = mapDefinitionString.split(' ')[0].split('-');
      const mapRanges: MappingRange[] = mappingRangeLines.split('\n')
        .filter((line) => line.length > 0)
        .map((mappingRangeLine: string): MappingRange => {
          const [destinationStart, sourceStart, rangeLength]: number[] =
            mappingRangeLine.split(' ').map((numberString) => Number(numberString));
        ...
        // Sort MappingRanges based on sourceStart (this will help later)
        .sort(({sourceStart: sourceStartA}, {sourceStart: sourceStartB}) => sourceStartA - sourceStartB);
  ...
  // Convert each seed number to a location number.

  // Each key converts to exactly one other key. Let's get a map of which keys map to which other keys
  const almanacKeys: {[key: string]: string} = almanacMaps
    .map(({sourceName, destinationName}) => [sourceName, destinationName])
    .reduce((keyMap, [sourceName, destinationName]) => {
      keyMap[sourceName] = destinationName;
      return keyMap;
    }, {} as {[key: string]: string});
  
  // Map source to each relevant almanac
  const sourceKeyToAlamanac: {[key: string]: AlmanacMapping} = almanacMaps
    .map((almanacMap): [string, AlmanacMapping] => [almanacMap.sourceName, almanacMap])
    .reduce((keyMap, [sourceName, almanacMap]) => {
      keyMap[sourceName] = almanacMap;
      return keyMap;
    }, {} as {[key: string]: AlmanacMapping});

  function getDestinationRangesFromSourceRange(
    sourceRange: NumberRange,
    currentMapping: AlmanacMapping,
  ): NumberRange[] {
    // Each source range could cover multiple distinct destination ranges
    const destinationRanges: NumberRange[] = [];

    // Find all overlapping "mapped" ranges
    for (let mappingRange of currentMapping.mapRanges) {
      // Check for no overlap
      if (mappingRange.sourceStart + mappingRange.rangeLength < sourceRange.start) {
        continue;
      }
      if (mappingRange.sourceStart > sourceRange.start + sourceRange.length) {
        // Past the end of the range
        break;
      }

      // Find boundary of range overlap
      const sourceRangeStart = Math.max(mappingRange.sourceStart, sourceRange.start);
      // Subtract 1 because ranges are inclusive
      const sourceRangeEnd = Math.min(
        mappingRange.sourceStart + mappingRange.rangeLength,
        sourceRange.start + sourceRange.length) - 1;

      const destinationRangeStart = convertSourceToDestination(sourceRangeStart, [mappingRange]);
      // Add 1 because the length is inclusive of start value
      const destinationRangeLength = (sourceRangeEnd - sourceRangeStart) + 1;
      
      destinationRanges.push({
        start: destinationRangeStart,
        length: destinationRangeLength,
      });
    }

    // Find all non-overlapping "gap" ranges
    let lastRangeValue = sourceRange.start;
    for (let mappingRange of currentMapping.mapRanges) {
      if (sourceRange.start + sourceRange.length < lastRangeValue) {
        // Past the end of the range
        break;
      }

      // Left gap: lastRangeValue ---- mappingRange.sourceStart
      // Check if sourceRange start or sourceRange end are between these values
      // Left boundary
      const sourceRangeStart = Math.max(sourceRange.start, lastRangeValue);
      // Right boundary (NOTE: I spent half an hour debugging to fix this line specifically...)
      const sourceRangeEnd = Math.min(sourceRange.start + sourceRange.length, mappingRange.sourceStart) - 1;

      if (sourceRangeEnd > sourceRangeStart) {
        // Skip conversion because there's no mapping
        destinationRanges.push({
      ...
      // Update lastRangeValue
      lastRangeValue = mappingRange.sourceStart + mappingRange.rangeLength - 1;
    }

    if (sourceRange.start + sourceRange.length > lastRangeValue) {
      // Final gap: lastRangeValue ---- sourceRange.start + sourceRange.length
      // Check if sourceRange start or sourceRange end are between these values
      // Left boundary
      const sourceRangeStart = Math.max(sourceRange.start, lastRangeValue);
      // Right boundary is the end of the sourceRange
      const sourceRangeEnd = sourceRange.start + sourceRange.length - 1;
      // Skip conversion because there's no mapping
      destinationRanges.push({
        start: sourceRangeStart,
        length: (sourceRangeEnd - sourceRangeStart) + 1,
  ...
  // Start with 'seed' key and end with 'location' key
  const START_KEY = 'seed';
  const END_KEY = 'location';

  let sourceRanges: NumberRange[] = seedRanges;
  let currentKey = START_KEY;

  while (currentKey !== END_KEY) {
    const currentMapping: AlmanacMapping = sourceKeyToAlamanac[currentKey];
    sourceRanges = sourceRanges
      .flatMap((range) => getDestinationRangesFromSourceRange(range, currentMapping));

    currentKey = almanacKeys[currentKey];
  }

  // Return lowest location value
  const locationValues = sourceRanges.map(({start}) => start);
  const startingMin = locationValues[0];
  return locationValues.reduce((min, locationValue) => locationValue < min ? locationValue: min, startingMin);
}

const input = `seeds: 3121711159 166491471 3942191905 154855415 3423756552 210503354 2714499581 312077252 1371898531 165242002 752983293 93023991 3321707304 21275084 949929163 233055973 3626585 170407229 395618482 226312891
...
console.log(lowestLocationNumberForSeeds(input));
